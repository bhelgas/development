...
client.on(Events.MessageCreate, async message => {
	...
	try {
		...
		if (useModelSystemMessage && modelInfo.system) {
			systemMessages.push(modelInfo.system.trim());
		}
		
		if (useCustomSystemMessage) {
			systemMessages.push(customSystemMessage.trim());
		}
		...
		if (userInput.startsWith("!")) {
			...
			switch (cmd) {
				...
				case "clear":
					...
					break;
				case "help":
					await message.reply({content: "Temp"});
				case "?":
					await message.reply({content: "Temp"});
				case "h":
					break;
				...
				case "":
					break;
				default:
					await message.reply({ content: "Unknown command, type `!help` (Nepolyie Bot must be in the server) for a list of commands" });
					break;
			}
			return;
		}
